...
inline void Element::put_height_mom(double pile_height, double volf, double xmom, double ymom) {
	...
	if (pile_height > 0.)
		prev_state_vars[0] = state_vars[0] = prev_state_vars[4] = state_vars[4] = -5 * min_dx;
	else
		prev_state_vars[0] = state_vars[0] = 5 * min_dx;
...
inline double* Element::get_phi_slope() {
	return phi_slope;
}
;
