			...
			if (timeprops->iter == 0) {
				...
				for (iplane = 0; iplane < num_planes; iplane++)
					fprintf(fp, "plane %d endpoints are (%16.10g,%16.10g) (%16.10g,%16.10g)\n", iplane + 1,
					    discharge->planes[iplane][0] * (matprops->LENGTH_SCALE),
...
void OUTPUT_ADAM_STATS(HashTable* El_Table, MatProps* matprops_ptr, TimeProps* timeprops_ptr,
	...
	if (myid == 0) {
		...
		fprintf(fp,
		    ...
		    xyh_vmax[0] * matprops_ptr->LENGTH_SCALE, xyh_vmax[1] * matprops_ptr->LENGTH_SCALE,
		    ...
		    xyv_hmax[0] * matprops_ptr->LENGTH_SCALE, xyv_hmax[1] * matprops_ptr->LENGTH_SCALE,
