		...
		while (entryp) {
			...
			if ((EmTemp->get_adapted_flag() > 0)
				...
				if (*(EmTemp->get_state_vars() + 1) > GEOFLOW_TINY) {
					...
					//if (*(EmTemp->get_state_vars()+1)>0.0001){
...
//printf("the value of height=%f .................phi=%f\n", *(EmTemp->get_state_vars()+1),*(EmTemp->get_state_vars()));
					...
					if (evalue > 10000000.) {
						...
						fprintf(stderr,
								...
								solid :(%e, %e) \n \
								fluid :(%e, %e) \n \
								for pile height %e curvature=%e phi=%e k_xy=%f gravity=%f (x,y)=(%e,%e)\n",
								evalue, myid, *(EmTemp->get_state_vars() + 2), *(EmTemp->get_state_vars() + 3),
								*(EmTemp->get_state_vars() + 4), *(EmTemp->get_state_vars() + 5),
								*(EmTemp->get_state_vars() + 1), maxcurve, *(EmTemp->get_state_vars()),
								*(EmTemp->get_kactxy()), *(EmTemp->get_gravity()), *(EmTemp->get_coord()),
								*(EmTemp->get_coord() + 1));
						exit(1);
					}
